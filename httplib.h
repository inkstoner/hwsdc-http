...
public:
  ...
  virtual ssize_t ssread(char *ptr, size_t size) = 0;
  virtual ssize_t sswrite(const char *ptr, size_t size) = 0;
  virtual void get_remote_ip_and_port(std::string &ip, int &port) const = 0;

  template <typename... Args>
  ssize_t write_format(const char *fmt, const Args &... args);
  ssize_t sswrite(const char *ptr);
  ssize_t sswrite(const std::string &s);
};
...
public:
  ...
  ssize_t ssread(char *ptr, size_t size) override;
  ssize_t sswrite(const char *ptr, size_t size) override;
...
#else
    #ifdef HWSDC
    //by inkstoner 1 	//创建socket，指定通信协议为AF_UNIX,数据方式SOCK_STREAM
...
inline socket_t create_client_socket(const char *host, int port,
  ...
  auto sock = create_socket(
      ...
      [&](socket_t sock, struct addrinfo &ai) -> bool {
        ...
        #ifdef HWSDC
        //by inkstoner //将地址绑定到nginx监听的域套接字/tmp/http_proxy_connect.socket
...
//          printf("The client begin to connect service\n");
          //建立连接
         auto ret = connect(sock, (struct sockaddr *)&address, len);
//          printf("The connect result from service is %d, %s\n",ret, strerror(errno));

          //要发给域套接字的请求信息
          //要发给域套接字的请求信息
          char connectUrl[] = "CONNECT %s:%d HTTP/1.1\r\nHost:%s:%d\r\n\r\n";
          //存放发送的信息
          char send_buf_connect[SEND_LENGTH];
          //存放收到的信息
          char ch_recv_connect[MAX_LENGTH];
          //用sprintf事先把消息写到send_buf_connect
          sprintf(send_buf_connect,connectUrl,host,port,host,port);
//          printf("The connect message is %s and length is %d\n",send_buf_connect,strlen(send_buf_connect));
          //发送请求数据
          int byte;
          if((byte=write(sock, send_buf_connect, strlen(send_buf_connect)))==-1)
          {
              perror("write");
              exit(EXIT_FAILURE);
          }
          //接收返回数据
//          if((byte=read(sock,&ch_recv_connect,MAX_LENGTH))==-1)
//          {
//              perror("read");
//              exit(EXIT_FAILURE);
//          }
    ...
    if (line_reader.end_with_crlf()) {
      // Blank line indicates end of headers.
        ...
        if (line_reader.size() == 2) {
            if (headers.size() == 1 && (headers.begin()->first == "Proxy-agent")) {
                continue;
            } else {
                break;
            }
...
inline bool read_content_with_length(Stream &strm, uint64_t len,
  ...
  while (r < len) {
    ...
    auto n = strm.ssread(buf, (std::min)(read_len, CPPHTTPLIB_RECV_BUFSIZ));
    if (n <= 0) { return false; }
...
inline bool read_content_without_length(Stream &strm,
  ...
  uint64_t r = 0;
  for (;;) {
    auto n = strm.ssread(buf, CPPHTTPLIB_RECV_BUFSIZ);
    if (n < 0) {
      return false;
    } else if (n == 0) {
      return true;
...
inline bool write_data(Stream &strm, const char *d, size_t l) {
  size_t offset = 0;
  while (offset < l) {
    auto length = strm.sswrite(d + offset, l - offset);
...
inline bool write_multipart_ranges_data(Stream &strm, const Request &req,
  ...
  return process_multipart_ranges_data(
      req, res, boundary, content_type,
      [&](const std::string &token) { strm.sswrite(token); },
      [&](const char *token) { strm.sswrite(token); },
      [&](size_t offset, size_t length) {
...
inline ssize_t Stream::sswrite(const char *ptr) {
  return sswrite(ptr, strlen(ptr));
}

inline ssize_t Stream::sswrite(const std::string &s) {
  return sswrite(s.data(), s.size());
}
...
inline ssize_t Stream::write_format(const char *fmt, const Args &... args) {
  ...
  if (n >= buf.size() - 1) {
    ...
    return sswrite(&glowable_buf[0], n);
  } else {
    return sswrite(buf.data(), n);
  }
}
...
inline ssize_t SocketStream::ssread(char *ptr, size_t size) {
...
inline ssize_t SocketStream::sswrite(const char *ptr, size_t size) {
  ...
  return handle_EINTR([&]() {
      ...
      {
          int byte = 0;
          if ((byte = write(sock_, ptr, size)) == -1)
          {
              printf("write\n");
              return -1;
          }
...
inline ssize_t BufferStream::ssread(char *ptr, size_t size) {
#if defined(_MSC_VER) && _MSC_VER <= 1900
...
inline ssize_t BufferStream::sswrite(const char *ptr, size_t size) {
...
inline bool Server::write_response(Stream &strm, bool close_connection,
  ...
  auto &data = bstrm.get_buffer();
    strm.sswrite(data.data(), data.size());

  // Body
  auto ret = true;
  if (req.method != "HEAD") {
    if (!res.body.empty()) {
      if (!strm.sswrite(res.body)) { ret = false; }
...
Server::create_server_socket(const char *host, int port, int socket_flags,
  ...
  return detail::create_socket(
      ...
      [](socket_t sock, struct addrinfo &ai) -> bool {
        ...
        //创建域套接字
        bind(sock, (struct sockaddr *)&server_address, server_len);
        //监听请求
        ::listen(sock, 5);
#else
        if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) {
...

  {
    ...
    while (svr_sock_ != INVALID_SOCKET) {
      ...
      if (idle_interval_sec_ > 0 || idle_interval_usec_ > 0) {
        ...
        socket_t sock = accept(svr_sock_,(struct sockaddr *)&client_address, (socklen_t *)&client_len);
#else
